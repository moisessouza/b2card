...
def gerar_arquivo_aprovacao(request, pacote_itens_id):
    ...
    for v in pacote_itens:
        ...
        horas_contratadas = ParcelaFase.objects.filter(parcela__demanda__id = demanda_id,parcela__parcelafase__medicao__valor_hora__id=valor_hora_id, parcela__parcelafase__fase__fase__id=fase__id).aggregate(horas_contratadas= Sum('parcela__parcelafase__medicao__quantidade_horas'))['horas_contratadas']
        ...
        horas_a_faturar = ParcelaFase.objects.filter(parcela__status__in = ['PA'],
                                parcela__demanda__id = demanda_id,parcela__parcelafase__medicao__valor_hora__id=valor_hora_id).aggregate(quantidade_horas = Sum('parcela__parcelafase__medicao__quantidade_horas'))['quantidade_horas']                       
        
        valor_a_faturar = ParcelaFase.objects.filter(parcela__status__in = ['PA'], 
                                parcela__demanda__id = demanda_id,parcela__parcelafase__medicao__valor_hora__id=valor_hora_id).aggregate(valor_total = Sum('parcela__parcelafase__medicao__valor_total'))['valor_total']
...
"""def gerar_arquivo_aprovacao(request, pacote_itens_id):
    ...
    medicoes = Medicao.objects.filter(parcela_fase__parcela__pacote_itens__id=pacote_itens_id).values('parcela_fase__parcela__demanda__id', 'parcela_fase__parcela__demanda__nome_demanda', 'parcela_fase__parcela__demanda__codigo_demanda', 
                           'parcela_fase__fase__fase__descricao', 'parcela_fase__fase__fase__id', 'valor_hora__id', 'valor_hora__descricao')
    
    for v in medicoes:
        
        valor_hora_id = v['valor_hora__id']
        demanda_id = v['parcela_fase__parcela__demanda__id']
        #medicao_id = v['id']
        fase__id = v['parcela_fase__fase__fase__id']
        
        horas_contratadas = Medicao.objects.filter(parcela_fase__parcela__demanda__id = demanda_id,valor_hora__id=valor_hora_id, parcela_fase__fase__fase__id=fase__id).aggregate(horas_contratadas= Sum('quantidade_horas'))['horas_contratadas']
        horas_ja_faturadas = Medicao.objects.filter(parcela_fase__parcela__status__in = ['FA', 'PG'], 
                                parcela_fase__parcela__demanda__id = demanda_id,valor_hora__id=valor_hora_id, parcela_fase__fase__fase__id=fase__id).aggregate(horas_ja_faturadas= Sum('quantidade_horas'))['horas_ja_faturadas']
                                
        saldo_a_faturar = Medicao.objects.filter(parcela_fase__parcela__status__in = ['PA', 'FA', 'PG'], 
                                parcela_fase__parcela__demanda__id = demanda_id,valor_hora__id=valor_hora_id, parcela_fase__fase__fase__id=fase__id).aggregate(saldo_a_faturar= Sum('quantidade_horas'))['saldo_a_faturar']
        
        valor_por_hora = Medicao.objects.filter(parcela_fase__parcela__status__in = ['PA'],
                                parcela_fase__parcela__demanda__id = demanda_id,valor_hora__id=valor_hora_id).annotate(valor_por_hora=F('valor_total')/F('quantidade_horas'))[0].valor_por_hora
                                
        horas_a_faturar = Medicao.objects.filter(parcela_fase__parcela__status__in = ['PA'],
                                parcela_fase__parcela__demanda__id = demanda_id,valor_hora__id=valor_hora_id).aggregate(quantidade_horas = Sum('quantidade_horas'))['quantidade_horas']                       
        
        valor_a_faturar = Medicao.objects.filter(parcela_fase__parcela__status__in = ['PA'], 
                                parcela_fase__parcela__demanda__id = demanda_id,valor_hora__id=valor_hora_id).aggregate(valor_total = Sum('valor_total'))['valor_total']
    ...
    arquivo_gerado = docxgen.gerar_arquivo_aprovacao(arquivo_template, medicoes)
